			...
			datum = {
				content:(
					<div key={filename} className={styles.file}>
						<img src={src} alt="" />
						{targetArray.length > 1 && <div className={styles.minusButton}>
		...
		if (!isEmpty(files)) {
			const sortFiles = (sortOrder, originalFiles) => {
				...
				for (let i = 0; i < sortOrder.length; i++) {
					const reorderedItem = originalFiles.find( item => sortOrder[i] === item.name )
			...
			const sortDefaultValue = (sortOrder, originalValue) => {
				const newOrder = []
				for (let i = 0; i < sortOrder.length; i++) {
					const reorderedItem = originalValue.find( content => sortOrder[i] === content.key )
					newOrder.push(reorderedItem)
				}
				return newOrder
			}
	...
	render() {
		...
		return (
			<div>
	 		    ...
	 		    <Dropzone accept='image/jpeg' onDrop={acceptedFiles => this.onDrop(acceptedFiles)}>
