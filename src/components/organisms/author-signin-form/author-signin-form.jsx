		...
		this.setState({
			...
			sendMagicLink(email).then( data => {
				this.setState({ LOADING_STATE: 'loaded' }, () => {
					...
					if (data === 'resource not found') {						
						toastedNotes.notify(<Toast htmlMessage={`<em>${email}</em> is not an Author email.`} />)
						...
						const message = "<p>Check your email for your magic signin link. <strong>It could a take a few minutes to arrive</strong>. Be sure to check your spam folders!</p>"
						toastedNotes.notify(<Toast htmlMessage={message} fullWidth={true} />, { duration: TOAST_DURATION })
						const delayFormResetToPreventTooManySubmissions = setTimeout(() => {
			...
			.catch( error => {
				...
				this.setState({ LOADING_STATE: 'loaded' }, () => {
					...
					toastedNotes.notify(<Toast message={message} />, { duration: 3000 })
					const delayFormResetToPreventTooManySubmissions = setTimeout(() => {
						this.setState({ LOADING_STATE: 'unloaded', isEmailInputDisabled: false })
						clearTimeout(delayFormResetToPreventTooManySubmissions)
					}, 3000)
				})
			})
			...
			<div className={styles.window}>
				...
				<div className={styles.info}>
					<div className={styles.content}>
						...
						<p>...ye are a chosen generation, a royal priesthood, an holy nation, a peculiar people; that ye should shew forth the praises of him who hath called you out of darkness into his marvellous light...</p>
