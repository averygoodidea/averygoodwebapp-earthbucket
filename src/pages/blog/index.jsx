...
const BlogIndexPage = ({ data, location }) => {
  ...
  const blogTagNames = {}
  postExcerpts.forEach( ({ node }) => node.frontmatter.tags.forEach( tag => blogTagNames[tag] = true ))
...
export const query = graphql`
  query {
    allPostExcerpts: allMarkdownRemark(sort: {order: DESC, fields: frontmatter___date}, filter: {frontmatter: {published: {eq: true}}}) {
      edges {
        node {
          frontmatter {
            author
